  ...
  return {
    ...
    actions: {
      ...
      editUser: async (rut, data) => {
        ...
        try {
          const response = await fetch(`https://back-end-cafe-planta.vercel.app/user/edit/${rut}`, {
            method: "PUT",
            headers: {
      ...
      deleteUser: async (rut, adminPassword) => {
        const { token, admin, queriedUsers } = getStore();
        try {
          const response = await fetch(`https://back-end-cafe-planta.vercel.app/user/delete/${rut}`, {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
          ...
          if (response.ok) {
            ...
            // Actualizar la lista de usuarios en el store
            const updatedUsers = queriedUsers.filter((user) => user.rut !== rut);
            setStore({ queriedUsers: updatedUsers });
      ...
      createUser: async (userData) => {
        const { token } = getStore();
        try {
          const response = await fetch("https://back-end-cafe-planta.vercel.app/user/create-user", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`
      ...
      fetchRoles: async () => {
        try {
          const response = await fetch("https://back-end-cafe-planta.vercel.app/role/", {
            method: "GET",
            headers: { "Content-Type": "application/json" },
            credentials: "include"
          });
          ...
          if (!response.ok) {
            const errorData = await response.json();
            console.error("Error al obtener los roles:", errorData);
            return;
          }
      
          const rolesData = await response.json();
          setStore({ roles: rolesData });
        } catch (error) {
          console.error("Error en fetchRoles:", error);
        }
      },
      deleteDiningArea: async (areaId, adminPassword) => {
        const { token, admin, diningAreas } = getStore();
        try {
          const response = await fetch(`https://back-end-cafe-planta.vercel.app/dining_area/delete/${areaId}`, {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`
          ...
          if (response.ok) {
            // Actualizar las mesas en el store
            const updatedAreas = diningAreas.filter(a => a.id !== areaId);
            setStore({ diningAreas: updatedAreas });
            return { success: true, message: result.message };
          } else {
            return { success: false, message: result.error || "Error desconocido" };
          }
          ...
          console.error("Error en deleteDiningArea:", error);
          return { success: false, message: "Error de conexión al servidor" };
        }
      },
